...
export default function TradeForm({ type }: TradeFormProps) {
  ...
  const [formState, setFormState] = useState<{ amount: string; slippage: number }>({
    amount: "",
  ...
  const { mutate: submitOrder, isPending } = useMutation({
    mutationFn: async (formData: FormData) => {
      ...
      const slippage = parseFloat(formData.get("slippage") as string) / 100 || 0;
    ...
    onSuccess: async (res) => {
      if (res?.status === "ok") {
        const statuses: { [key: string]: string }[] = res?.response?.data?.statuses;
        ...
        statuses.forEach(async (status) => {
          if (status?.error) {
            ...
            await Promise.all([
              ...
              queryClient.invalidateQueries({ queryKey: ["midPrice", "@1035"] }),
  ...
  const userBalances = {
    usdc: balanceData?.balances?.find((balance) => balance.coin === "USDC")?.total ?? 0,
    hype: balanceData?.balances?.find((balance) => balance.coin === "HYPE")?.total ?? 0,
  };

  const calculateTotalAmount = (amount: string) => {
    if (!midPriceData || !amount) return 0;
    return isBuy
      ? Math.floor((parseFloat(amount) / midPriceData) * Math.pow(10, tokenDetails?.szDecimals || 6)) /
  ...
  const orderValue = midPriceData ? `${(totalAmount * midPriceData).toFixed(2)} USDC` : "Calculating...";
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="mb-4">{isBuy ? "Buy $HYPE" : "Sell $HYPE"}</CardTitle>
        <CardDescription>
          <dl className="grid grid-cols-2 gap-y-1">
            <dt className="text-sm font-medium leading-none">USDC Balance</dt>
            <dd className="text-sm text-muted-foreground text-right">{userBalances.usdc} USDC</dd>
            <dt className="text-sm font-medium leading-none">HYPE Balance</dt>
            <dd className="text-sm text-muted-foreground text-right">{userBalances.hype} HYPE</dd>
          </dl>
        </CardDescription>
      </CardHeader>
        ...
        <form ref={formRef} onSubmit={handleSubmit}>
          <div className="grid w-full items-center gap-4">
            <div className="space-y-1">
              <Label htmlFor="amount">Amount to {actionText} (in {isBuy ? "USDC" : "HYPE"})</Label>
              <Input id="amount" name="amount" onChange={handleAmountChange} value={formState.amount} />
            </div>

            <div className="space-y-1">
              <Label htmlFor="slippage">Slippage</Label>
              <Input id="slippage" name="slippage" defaultValue={DEFAULT_SLIPPAGE} />
            </div>
          </div>

          <dl className="grid grid-cols-2 gap-y-1 mt-6">
            <dt className="text-sm font-medium leading-none">Market Value</dt>
            <dd className="text-sm text-muted-foreground text-right">1 HYPE = {midPriceData} USDC</dd>

            <dt className="text-sm font-medium leading-none">Estimated {isBuy ? "HYPE to buy" : "USDC to receive"}</dt>
            <dd className="text-sm text-muted-foreground text-right">
              {isBuy ? totalAmount.toFixed(tokenDetails?.szDecimals || 6) : (totalAmount * midPriceData).toFixed(2)}
            </dd>

            <dt className="text-sm font-medium leading-none">Order Value</dt>
            <dd className="text-sm text-muted-foreground text-right">{orderValue}</dd>
          </dl>
