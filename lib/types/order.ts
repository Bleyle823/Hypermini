...
export type TriggerOrderTypeWire = { triggerPx: string; isMarket: boolean; tpsl: Tpsl };

export type OrderTypeWire = { limit?: LimitOrderType; trigger?: TriggerOrderTypeWire };

export type OrderWire = { a: number; b: boolean; p: string; s: string; r: boolean; t: OrderTypeWire; c?: string };
...
class Cloid {
  ...
  private _validate(): void {
    if (!this._rawCloid.startsWith("0x")) throw new Error("cloid is not a hex string");
    if (this._rawCloid.slice(2).length !== 32) throw new Error("cloid is not 16 bytes");
  }
...
function orderTypeToWire(orderType: OrderType): OrderTypeWire {
  if ("limit" in orderType) {
    ...
    return { trigger: { isMarket: orderType.trigger.isMarket, triggerPx: floatToWire(orderType.trigger.triggerPx), tpsl: orderType.trigger.tpsl } };
  }
  throw new Error("Invalid order type");
}

export const orderRequestToOrderWire = (order: OrderRequest, asset: number): OrderWire => {
