...
import { OrderRequest, orderWiresToOrderAction, orderRequestToOrderWire } from "@/lib/types/order";
...
export async function submitMarketOrder(orderDto: Order, pk: `0x${string}`) {
  try {
    const pairIndex = await requestAssetId(orderDto.baseToken, orderDto.quoteToken);
    if (pairIndex > -1) {
      const nonce = Date.now();
      const account = privateKeyToAccount(pk);
      const calculatedAssetId = 10000 + pairIndex;
      const orderWire = orderRequestToOrderWire(orderDto.order, calculatedAssetId);
      const orderAction = orderWiresToOrderAction([orderWire]);
      const signature: Signature = await signStandardL1Action(orderAction, account, null, nonce);
      ...
      const orderRequest = {
        action: {
          ...
          builder: { b: config.builderAddress.toLowerCase(), f: 1 },
        },
        nonce: nonce,
        signature: signature,
        vaultAddress: null,
...
type SpotMetaResponse = {
  universe: Array<{ tokens: [number, number]; name: string; index: number; isCanonical: boolean }>;
  tokens: Array<{ name: string; szDecimals: number; weiDecimals: number; index: number; tokenId: string; isCanonical: boolean }>;
};

async function requestAssetId(baseToken: string, quoteToken: string): Promise<number> {
  const response = await axios.post(`/info`, { type: "spotMeta" });
  if (response.data) {
...
export function constructOrder({ amount, isBuy, slippage, midPrice, tokenDecimals }: ConstructOrderParams) {
  const priceWithSlippage = calculateSlippagePrice(midPrice, isBuy, slippage, true);
  let size = isBuy ? parseFloat(amount) / midPrice : parseFloat(amount);
  size = Math.floor(size * Math.pow(10, tokenDecimals)) / Math.pow(10, tokenDecimals);
  ...
  return { baseToken: "HYPE", quoteToken: "USDC", order: orderRequest };
}

export const calculateSlippagePrice = (price: number, isBuy: boolean, slippage: number, isSpot: boolean): string => {
